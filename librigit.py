...
argparser = argparse.ArgumentParser(
    description="This is a python ReImplimentation of GIT (RIGIT)")
...
class GitRepository(object):
    ...
    def __init__(self, path, force=False):
        self.worktree = path
        self.gitdir = os.path.join(path, ".rigit")
...
def repo_find(path=".", required=True):
    ...
    if os.path.isdir(os.path.join(path, ".rigit")):
...
def tag_create(repo, name, ref, create_tag_object=False):
    ...
    if create_tag_object:
        ...
        tag.kvlm[b'tagger'] = b'Rigit <rigit@example.com>'
        tag.kvlm[None] = b"A tag generated by rigit, which won't let you customize the message!"
...
def index_read(repo):
    ...
    assert version == 2, "rigit only supports index file version 2"
    ...
    for e in index.entries:
        ...
        if args.verbose:
            ...
            print("  created: {}.{}, modified: {}.{}".format(
                datetime.fromtimestamp(e.ctime[0]), e.ctime[1], datetime.fromtimestamp(e.mtime[0]), e.mtime[1]))
...
def add(repo, paths, delete=True, skip_missing=False):
    ...
    for path in paths:
        ...
        if not (abspath.startswith(worktree) and os.path.isfile(abspath)):
            raise Exception(
                "Not a file, or outside the worktree: {}".format(paths))
